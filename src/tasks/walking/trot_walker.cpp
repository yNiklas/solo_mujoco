...
TrotWalker::TrotWalker(std::shared_ptr<rclcpp::Publisher<std_msgs::msg::Float64MultiArray>> jointPositionPublisher) : jointPositionPublisher_(jointPositionPublisher) {}

IKSolver::FootTargets TrotWalker::computeFootTargets(const double timestamp) {
    double t = timestamp;
...
void TrotWalker::execute(const double timestamp) {
...
void TrotWalker::startReturnToStableStand([[maybe_unused]] const double current_timestamp, [[maybe_unused]] const double desired_duration) {
    std_msgs::msg::Float64MultiArray msg;
    msg.data.clear();
...
void TrotWalker::returnToStableStand([[maybe_unused]] const double timestamp) {}

bool TrotWalker::returnedToStableStand([[maybe_unused]] const double timestamp) {
...
std::string TrotWalker::getName() const {
    return "TROT";
}
}
